Shader "Unlit/Wiggly"
{
    Properties
    {
        ...
        _UseSteps ("useSteps", Float) = 1.0
        _TimeScale ("TimeScale", Float) = 1.0
        _Scale ("NoiseScale", Float) = 1.0
        _Amplitude ("WiggleAmplitude", Float) = 0.1
    }
    SubShader
    {
        ...
        Pass
        {
            ...
            {
                float offset = _Time.y * _TimeScale;
                if (_UseSteps > 0)
                {
                    offset = floor(offset);
                }
                float2 scaledUv = i.uv * _Scale;
                float r = cnoise(float3(scaledUv.x, scaledUv.y, offset));
                float g = cnoise(float3(scaledUv.x + 100.f, scaledUv.y + 100.f, offset));

                float xDisplace = (r * 2.0 - 1.0) * _Amplitude;
                float yDisplace = (g * 2.0 - 1.0) * _Amplitude;

                float2 displacedUv = i.uv + float2(xDisplace, yDisplace);
                // sample the texture
                return tex2D(_MainTex, displacedUv);
                //return fixed4(r, g, 0.f, 1.f);
            }
            ENDCG
        }
    }
